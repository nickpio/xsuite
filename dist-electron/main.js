import electron, { app as app$1, ipcMain as ipcMain$1, BrowserWindow, Menu, MenuItem, shell as shell$1, nativeImage, Tray } from "electron";
...
function createWindow() {
  ...
  const iconPath = join(app$1.getAppPath(), "x-suite-icon.png");
  if (fs$1.existsSync(iconPath)) {
    mainWindow.setIcon(iconPath);
    if (process.platform === "darwin" && app$1.dock) {
      app$1.dock.setIcon(iconPath);
    }
  }
  if (isDev) {
...
function createTray() {
  const iconPath = join(app$1.getAppPath(), "x-suite-icon.png");
  const trayIcon = fs$1.existsSync(iconPath) ? nativeImage.createFromPath(iconPath).resize({ width: 16, height: 16 }) : nativeImage.createEmpty();
...
app$1.whenReady().then(() => {
  ipcMain$1.handle("load-workspaces", () => {
    return store.get("workspaces", []);
  });
  ipcMain$1.handle("save-workspace", (_event, workspace) => {
  ...
  ipcMain$1.handle("delete-workspace", (_event, name) => {
    const workspaces = store.get("workspaces", []);
...
app$1.on("web-contents-created", (_, contents) => {
